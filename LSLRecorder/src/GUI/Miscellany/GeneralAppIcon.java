...
public class GeneralAppIcon
{
	...
	public static ImageIcon Info( int size, Color color )
	{
		...
		if( size > 0 )
		{
			...
			Image c = imagenPoligono2D.crearImagenCircunferencia( 0, 0, size, thick, color, null );
						
			imagenPoligono2D.componerImagen( img, (img.getWidth( null ) - c.getWidth( null ) ) / 2
			...
			*/
			...
			Image imgI = imagenPoligono2D.crearImagenTexto( 0, 0, "i", fm, null, color, null );
	...
	public static ImageIcon Exit( int size, Color color )
	{
		...
		{	
			...
			Image img = imagenPoligono2D.crearLienzoVacio( w, w, null );
			
			int pI = (2 * w) / 3;
			
			imagenPoligono2D.crearImagenPoligonoRelleno( new int[] { pI, w, pI }
														, new int[] { 0, w / 2, w}
														, color, img);
			
			Image r = imagenPoligono2D.crearImagenRectangulo( pI - 2 * thickness 
															, ( 2 * w ) / 5
															, thickness
															, color
															, color );
			
			imagenPoligono2D.componerImagen( img, pI - r.getWidth( null ) + thickness * 2, w /2 - r.getHeight( null ) / 2, r );
			...
			imagenPoligono2D.crearImagenLineaPoligonal( new int[] { pI - 2 * thickness, thickness2, thickness2, pI - 2 * thickness}
	...
	public static ImageIcon getIconoAplicacion( int width, int height  )
	{
		...
		*/
		...
		Image icon = imagenPoligono2D.crearLienzoVacio( iconSize, iconSize, Color.WHITE);
		
		Image icon2 = imagenPoligono2D.crearLienzoVacio( iconSize, iconSize, null );
		icon2 = imagenPoligono2D.crearImagenLinea( iconSize / 2 - thick2,  arrowWidth / 2, iconSize / 2 - thick2, iconSize - arrowWidth / 2, 2 * thick2 , Color.BLACK, icon2 );
		icon2 = imagenPoligono2D.crearImagenLinea( arrowWidth / 2, iconSize / 2 - thick2, iconSize - arrowWidth / 2, iconSize / 2 - thick2, 2 * thick2, Color.BLACK, icon2 );
				
		icon = imagenPoligono2D.crearImagenArco( thick / 2, thick / 2, 
												iconSize - thick,
												iconSize - thick, 
												180, -90, 
		...
		Image aux = imagenPoligono2D.crearImagenPoligonoRelleno( new int[]{ 0, 0, arrowWidth / 2 },
																 new int[]{ 0, arrowWidth, arrowWidth / 2 }, 
																 Color.BLACK, 
																 null);
		
		icon = imagenPoligono2D.componerImagen( icon,  iconSize / 2 - 1, 0, aux);
		
		aux = imagenPoligono2D.crearImagenPoligonoRelleno( new int[]{ 0, arrowWidth, arrowWidth / 2 },
															 new int[]{  0, 0, arrowWidth / 2 }, 
															 Color.GREEN, 
															 null);

		icon = imagenPoligono2D.componerImagen( icon,  iconSize - arrowWidth, iconSize / 2 - 1, aux);		

		aux = imagenPoligono2D.crearImagenPoligonoRelleno( new int[]{ arrowWidth/2, arrowWidth/2, 0 },
															 new int[]{  0, arrowWidth, arrowWidth / 2 }, 
															 Color.CYAN.darker(), 
															 null);

		icon = imagenPoligono2D.componerImagen( icon,  iconSize / 2 - arrowWidth / 2, iconSize - arrowWidth, aux);		
		...
		icon = imagenPoligono2D.componerImagen( icon,  0, iconSize / 2 - arrowWidth / 2, aux);		
		
		//icon = imagenPoligono2D.crearImagenCircunferencia(0, 0, iconSize-1, thick2, Color.BLACK, icon );
		
		icon = imagenPoligono2D.crearImagenCirculo( arrowWidth, arrowWidth, iconSize - 2 * arrowWidth, Color.WHITE, icon );
		//icon = imagenPoligono2D.crearImagenCircunferencia( arrowWidth, arrowWidth, iconSize - 2 * arrowWidth, thick2, Color.BLACK, icon );
		
		icon = imagenPoligono2D.changeColorPixels( Color.WHITE, new Color( 255, 255, 255, 0), icon );
		
		icon = imagenPoligono2D.componerImagen( icon2, 0, 0, icon );
		
		icon = imagenPoligono2D.crearImagenCirculo( iconSize / 2 - iconSize / 8 - thick2, 
													iconSize / 2 - iconSize / 8, 
													iconSize / 4, Color.RED, icon );
		
		icon = imagenPoligono2D.crearImagenCircunferencia( iconSize / 2 - iconSize / 8 - thick2 * 2 + 1,  
	...
	public static ImageIcon Refresh( int width, int height, Color borderColor, Color fillColor )
	{
		...
		imagenPoligono2D.crearImagenArco( thickness / 2, thickness / 2, w - (thickness * 3 ) / 2 , h - 2 * l / 3 - thickness, 0, -270, thickness, borderColor, fillColor, img );
